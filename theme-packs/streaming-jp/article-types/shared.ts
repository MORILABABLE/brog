...
export interface TitleRule {
  ...
  /**
   * **主題のあとに置く問い**（`はどこで見れる？`）。省略なら検査しない。
   *
   * ■ なぜ問いを入れるのか（2026-09-06 追加）
   * 「終わった」ことだけを名乗るタイトルは、**事実としては正しいが
   * 読者が打った問いへの答えになっていない。**
   * 作品ページで同じ形を直したときの実測（docs/FUNNEL.md 3-2）はこう:
   *
   *     同じ掲載順位帯（4〜10位）で
   *       記事 `/posts/*`       CTR 9.9%
   *       作品ページ `/works/*`  CTR 2.9%   ← 461枚が「終了しました」で終わっていた
   *
   * 読者が実際に打っているのは「◯◯ 配信」「◯◯ どこで見れる」
   * 「◯◯ いつまで」「◯◯ netflix 配信終了」の4つの形（同 2-1）。
   * **問いと事実を1本に入れる**とその4つ全部に当たる。
   *
   * ★ **主題軸の「終了済み」にだけ渡すこと。** サービス軸（`【2026年8月】Disney+で…`）や
   *   終了予定の記事には置かない。理由は3つで、
   *
   *     1. 「Disney+はどこで見れる？」は文として成立しない
   *     2. 主語をまとまりに変えても、**48本の記事がコナンの記事に見える**
   *     3. **答えは表がもう持っている** — 終了記事の表には行ごとに
   *        「どこで観られるか」の1行が入る（site/plugins/rehype-availability.ts）
   *
   * ★ **2026-09-10 に一度サービス軸と `expiring` へ広げ、同じ日に戻した。**
   *   タイトルと節の見出しすべてに問いが並び、運用者の判断で撤回している
   *   （「問いかけが過剰です。表の他サービスへの表記で誘導は済んでいる」）。
   *   **同じ処方をもう一度当てないこと**（naming.md に経緯と実測がある）。
   *
   * ★ **問いは前に置く。** スマホの検索結果は全角30文字前後で切れるので、
   *   本数のうしろに置くと切れる位置に入る。主題の直後がぎりぎり収まる。
   */
...
export function titleIssues(title: string, ctx: ArticleContext, rule: TitleRule): VerifyIssue[] {
  ...
  // --- 問いの句（主題軸の「終了済み」だけ。TitleRule の注意書き参照） ---
  if (rule.questionClause && !title.includes(rule.questionClause)) {
    err(
      `タイトルに「${rule.questionClause}」がありません。すべて終わっている記事は、` +
        '「終わった」だけを名乗ると読者の問い（どこで観られるか）への答えになりません。' +
        `主題の直後に置いてください（templates/naming.md）。`,
    )
  }
...
/**
 * テンプレートの言葉が地の文に出たもの（`templates/writing.md` 1節・2026-09-13 追加）。
 *
 * > 「まとまり」「区間」「期間が、ほぼそのまま重なっています」など、
 * > 記事の構造を語る表現が目立ちます（読者が興味ない裏側を見せている）。
 * （2026-09-13 の添削・「007」シリーズ）
 *
 * **「束」「まとまり」「転機」「区間」はテンプレートが書き手に向けて使う言葉。**
 * どの解説をいくつ書くかを決めるための道具であって、読者に向けた言葉ではない。
 * 地の文に出ると、作品の話の代わりに**記事の設計図**を読ませることになる。
 *
 * ★ **見分け方はひとつ。** その文を消して事実が1つでも減るか。
 *   減らないなら、それは記事の作りの説明。
 */
...
/**
 * 2つの事実の重なりを書き手が指摘した言い回し（同・2026-09-13 追加）。
 *
 * 並べて書けば読者が気づく。「重なっています」と言った時点で、
 * **読者が自分で見つけるはずのものを先に奪っている。**
 *
 * ★ 作品の話で使う余地がある（「公開時期が重なる」など）ので warn に留める。
 */
const OVERLAP_TALK = ['まとまりは', 'この区間', '区間です', '重なっています'] as const
...
export function styleIssues(md: string): VerifyIssue[] {
  ...
  for (const phrase of TEMPLATE_VOCABULARY) {
    if (!text.includes(phrase)) continue
    issues.push({
      level: 'error',
      message:
        `「${phrase}」はテンプレートが書き手に向けて使う言葉です（templates/writing.md 1節）。` +
        '束・まとまり・転機・区間は解説の組み立て方を決める道具で、読者に見せるものではありません。' +
        '事実だけを書いてください。例:「主題歌は3作続けてアカデミー歌曲賞を受けています。」',
    })
  }

  for (const phrase of OVERLAP_TALK) {
    if (!text.includes(phrase)) continue
    issues.push({
      level: 'warn',
      message:
        `「${phrase}」は記事の構造を語っているように読めます（templates/writing.md 1節）。` +
        '2つの事実の重なりは書き手が指摘せず、並べて書いて読者に気づかせてください。' +
        '作品そのものを説明している文なら、この指摘は無視してよい。',
    })
  }
