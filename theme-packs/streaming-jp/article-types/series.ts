  ...
  buildPrompt(items, ctx) {
    ...
    const rows = items.map((e) => {
      ...
      return [
        ...
        /*
         * ★ 表の「出演者」欄はここから選ぶ。**網羅しない。**
         *   並び順は主演順ではないので、`peopleLine` の注意書きをそのまま渡す
         *   （選んだ人を「主演」と書かせないため。主演が言えるのはリサーチに
         *   　その記述がある作品だけ）。
         */
...
${OUTPUT_FORMAT}`
   ...
   **サービス列と状態列を省かないでください**（サイトが行のサービス名を読んでリンクを付けます）。`,
   ...
   ★ **キャラ名は素材にあるときだけ**です。いまの素材は声優の名前しか持っていないので、
     多くのアニメは **「—」** になります。キャラ名を推測で補ってはいけません。
   ★ 素材に \`出演\` の行が無い作品の欄も **「—」** にします。`,
      `**表の直後に「出演者」の見出しと箇条書きを置き、主演だけを書くこと。**
   読者がいちばん知りたいのは「誰が演じているシリーズなのか」の1点です。
   サイトがこの箇条書きを小さな枠にたたむので、**文章にせず、短く書きます**。
   ...
   ★ **頭のラベルは枠付きで、文字も色もサイトが決めます**
     （○があれば「配信中」＝青／△だけなら「有料で配信中」＝黄／×だけなら「取り扱いなし」＝赤）。
     **文字にも色にも触れないでください。** 本文で言い換えるのも禁止です。

   ★ **行も列も自分で書き足さないこと。** 表は「終了日 / 作品 / 状態 / 出演者 / サービス」の
