...
/**
 * 月を名乗る記事が、その月を過ぎたときに名乗り直す規則（`ArticleType.retire`）。
 *
 * **この層は文字列の中身を知らない。** 「配信終了」も「2026年8月」もテーマパックの言葉で、
 * 月の名乗り方（`${y}年${m}月`）を知っているのも記事タイプだけ。だから `monthOf` を渡してもらう。
 */
export interface RetireRule {
  /** この規則が当てはまる記事の目印。**このタグを持つ記事だけを見る** */
  readonly tag: string
  /** `tag` を差し替える先 */
  readonly becomes: string
  /**
   * 移る先のカテゴリ。省略すればカテゴリは変えない（タグだけ差し替える）。
   * ★ `site/src/config.ts` の `CATEGORIES` に在るものを返すこと。
   */
  readonly category?: Category
  /**
   * そのタグの並びが名乗っている月（`YYYY-MM`）。名乗っていなければ `undefined`。
   *
   * ★ **ここが「月次記事かどうか」の判定そのもの。** シリーズ記事は
   *   `配信終了` のタグを持つが年月のタグを持たない（`series.ts` の `tags()`）ので、
   *   ここで `undefined` になって対象から外れる。書き直しどきは `--refresh` が見る。
   */
  monthOf(tags: readonly string[]): string | undefined
}
...
export interface ArticleType {
  ...
  /**
   * **月を名乗らず、同じURLを書き直し続ける記事か**（2026-09-02 追加）。
   *
   * ■ 何のためにあるか
   * 月次記事は「2026年9月に何が終わるか」の**そのときの記録**で、
   * 9月30日を過ぎても書き直さない。**古くならない**（過去の月の話だから）。
   *
   * シリーズ記事（`series`）は逆で、`conan-movies` のような月を持たないURLを
   * 何か月も書き直す。終了予定だった作品が終了済みになれば、
   * **書き直さないかぎり記事だけが古い事実を言い続ける。**
   * この2つは「書き直しどきがあるか」がまったく違うので、区別が要る。
   *
   * ★ **CLI は月の名乗り方を知らない。** スラッグに `{年}-{月}-` が付くかどうかは
   *   テーマパックの決まりであって、パイプラインの決まりではない
   *   （`variants` や `flags` と同じ考え方）。だから記事タイプが宣言する。
   * ★ 宣言した記事タイプだけが `npm run write -- --refresh` の対象になる。
   *   宣言しなければ、いままでどおり取りこぼし（`core/coverage.ts`）だけで見る。
   */
  readonly evergreen?: boolean
  /**
   * **月を名乗る記事が、その月を過ぎたときに名乗り直す先**（2026-09-03 追加）。
   *
   * ■ `evergreen` の裏返し
   * シリーズ記事は**書き直して**事実に追いつく（`--refresh`）。
   * 月次記事は書き直さない。**本文は書いた月の記録として正しいまま**で、
   * 古くなるのは frontmatter の `category` と `tags` だけ。
   *
   *   タイトル `【2026年8月】…配信終了する作品32本` … 月を名乗っている。古くならない
   *   見出し   `## 8月11日：…が一斉に終了`          … 日付を名乗っている。古くならない
   *   本文     `8月9日時点では32本が…`               … 基準日を名乗っている。古くならない
   *   **バッジ `配信終了予定`（`category: leaving`）… 月を名乗っていない。9月には嘘になる**
   *
   * バッジは記事一覧・追従枠・左枠・記事ページ・サイト内検索の5か所に出る
   * （`site/src/config.ts` の `CATEGORIES`）。**記事の外に置かれる唯一の「いま」の主張**で、
   * 8月の記事が9月の本物の終了予定記事と同じアンバーで並ぶことになる。
   * だから直すのはこの2行だけでよく、本文には触らない。
   *
   * ■ 作品を1本も見なくていい理由
   * `leaving` の `select()` は「対象月に終了」かつ「終了日が未来」で絞っている。
   * つまり**その月が終わった時点で、載っている全作品の終了日が過ぎたことが確定する**。
   * 何件かを数え直す必要も、記事を読み直す必要もない（`core/retire.ts`）。
   *
   * ■ 規則は**タグで引く**
   * 記事タイプごとではなく `tag` の一致で当てる。同じタグを付ける他の記事タイプ
   * （特報の `--kind expiring` は `配信終了` を付ける）も、宣言なしでまとめて対象になる。
   * 読者が見ているのはタグとバッジであって記事タイプではないので、そちらに合わせる。
   *
   * ★ **配信開始側には宣言しないこと。** 「終了予定日が過ぎた＝終了した」は言えるが、
   *   「配信開始予定日が過ぎた＝始まった」は言えない。告知どおりに始まったかを
   *   当サイトは観測していない（`upcoming` の素材は告知であって観測ではない）。
   *   `配信開始予定` のタグが残るのは、**確かめていないことを書かない**ためで、直し忘れではない。
   */
  readonly retire?: RetireRule
  /**
  ...
  /**
   * その作品が本文に出ているか。**実装しなければ「題名がそのまま本文にあるか」で判定される。**
   *
   * ■ なぜ差し替えられる必要があるか（2026-08-30 追加）
   * 同じ映画がサービスごとに違う題で入っていることがある。
   *
   *     Netflix  名探偵コナン ベイカー街の亡霊
   *     U-NEXT   劇場版 名探偵コナン ベイカー街（ストリート）の亡霊
   *
   * 素材はサービスごとに1件ずつ持つ（終了日が別々に決まるため）が、
   * **表に2行出すと読者には別の作品に見える**ので、記事側は1行にまとめる。
   * 既定の判定のままだと、正しく書いた記事に毎回「取りこぼし」の警告が出る。
   *
   * ★ **どの題が同じ作品を指すのかは記事タイプだけが知っている**（`coveredBy` と同じ考え方）。
   *   パイプラインは判定を持たず、記事タイプが宣言したものを使うだけにする。
   */
  mentions?(item: ChangeEvent, body: string, items: ChangeEvent[]): boolean
  /**
   * 2つの観測が**同じ作品を指しているか**。既定は `work.id` の一致。
   *
   * ■ なぜ差し替えられる必要があるか（2026-09-02 追加）
   * 「終了しました」と書く前に他社に生きている観測が無いかを見る検査
   * （`core/cross-service.ts`）が、サービスをまたいで作品を突き合わせる。
   * ところが**同じ映画がサービスごとに違うIDで入っている**。
   *
   *     Netflix  名探偵コナン ベイカー街の亡霊                       （配信APIの数値ID）
   *     U-NEXT   劇場版 名探偵コナン ベイカー街（ストリート）の亡霊   （U-NEXT の自前ID）
   *
   * IDだけで突き合わせると、この2つは**別作品として扱われる**。
   * どの題が同じ作品を指すのかは記事タイプだけが知っているので
   * （`mentions` とまったく同じ理由）、判定はこちらに置く。
   *
   * ★ 実装しない記事タイプは `work.id` の一致で判定される。**それで安全側**
   *   （取りこぼしはするが、別作品を同じだと言うことはない）。
   */
  sameWork?(a: ChangeEvent, b: ChangeEvent): boolean
  /**
   * タイプ固有の検証。
   *
...
/**
 * バリアントを指すCLIフラグ名。既定は `genre`（ジャンル別記事が元の形だったため）。
 *
 * ★ CLI（`cli/write.ts`）と控えからのコマンド組み立て（`core/article-log.ts`）の
 *   両方が要るので、記事の抽象と同じ場所に置く。既定値が2か所にあると、
 *   一覧に出る形と貼れるコマンドの形が静かにずれる。
 */
export function variantFlag(type: ArticleType): string {
