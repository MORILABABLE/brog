 ...
 *
 * ★ **この制約は `/changes` の性質であって、APIの限界ではない**（2026-09-05 調査）。
 *   `/shows/search/filters` が返す `streamingOptions` は在庫そのもので、
 *   そちらを根拠にすれば「配信中」と書ける。
 *   **ただし取り込むなら守る条件が4つある**（`addon` を見放題と書かない、など）。
 *   設計と保留の理由は docs/CROSS-SERVICE.md。**ここを在庫の判定に作り替えないこと。**
 *
 * ■ 同じ作品かの判定
