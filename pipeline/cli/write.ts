...
function rewriteSection(previous: { slug: string; body: string; reason?: string }): string {
  /*
   * ★ **表の形がテンプレートより古い版だけに出す注意書き**（2026-09-08 追加）。
   *
   * 表の列を「評価」から「出演者」に入れ替えた（`templates/writing.md` 6節）。
   * だがこの節は「手を入れるのは事実が変わった箇所だけ」と強く言っているので、
   * **何も足さないと、書き直しても古い列がそのまま残る。**
   * 公開済み25本が書き直しのたびに旧い形で再生産されることになる。
   *
   * ★ **前の版に「評価」の列があるときだけ出す。** 移行が済んだ記事には出ないので、
   *   25本を書き直し終えたらこの注意書きは自然に消える（消し忘れが起きない）。
   */
  const legacyRatingColumn = /\|\s*評価\s*\|/.test(previous.body)
    ? `
## ★ 前の版の表は古い形です（列を差し替えてください）

前の版の表には **「評価」の列**があります。**いまのテンプレートでは廃止した列**です。

- **「評価」の列を「出演者」の列に置き換えてください。** 位置はそのまま（作品名の右）
- 出演者の欄は**主演と助演を1名ずつ（計2名まで）**。素材の \`出演\` の行から選びます
- 素材に \`出演\` の行が無い作品の欄は **「—」** にします（推測で埋めない）
- **評価スコアは記事のどこにも残さないでください。** 表からも地の文からも消します

★ **これは「事実が変わった箇所」ではありませんが、直してください。**
  下の「手を入れるのは次の2つだけ」の**例外**です。
  表の形が記事ごとに違うと、読者にも書き手にも一貫して見えません。
`
    : ''
