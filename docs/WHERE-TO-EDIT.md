...
frontmatter の `draft` を `true` にする。**ファイルは残り、いつでも戻せる。**
いちばん簡単なのは GitHub の **Actions → 「記事の公開／非公開」→ Run workflow**。
手順と注意（404になること）は [EDITING.md の「記事を非公開にする」](./EDITING.md#記事を非公開にする消さずに隠す)。
