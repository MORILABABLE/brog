...
```
┌──────────────┐
│ [絵] 配信終了予定 │  ← 常設ページのカード。1ページ1枚。広告ではない
│      Netflix   │
├──────────────┤
│ [絵] 新着配信    │
│      Netflix   │
...
**この2種類を混ぜてはいけない。** まとめて `aria-label="広告"` にすると
常設ページのカードまで広告と称することになり、逆にPR表記を外すと
景品表示法（ステマ規制）に触れる。

カードのバッジは記事カードと同じ `.badge[data-category]` を使っている。
サイト内の他のカードと同じ語彙で見せるため。

いま見ているページ自身のカードは自動で除かれる。

> **1枚にまとめない。** 当初「いつでも最新」という見出しの下にリンクを
> 箇条書きしていたが、サービスごとの見分けがつかず、
> サイト内の他のカードから浮いていた（2026-08-23 に作り直し）。
