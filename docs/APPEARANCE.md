...
**版が2つある。** ポスターが揃った節は右にポスター、
揃わない節は**従来どおり文字だけ**のカードになる。
どちらも枠・日付・見出し・作品の選び方は同じなので、読者が受け取る情報は変わらない。

| | 見た目 |
|---|---|
| ポスターあり | 左に日付・見出し、右にポスター2枚＋作品名・公開年 |
| ポスターなし | 左に日付・見出し、下に作品名（大）＋公開年 |

ポスターは**その節の2本ぶんが揃ったときだけ**使う。
1枚だけ欠けた状態で並べると、空の板が残って事故に見えるため。

ポスターが付かないのは次の場合（実測：37枚中7枚が文字だけの版）。

- **U-NEXT 由来の作品**（スクレイピングで集めているので画像が無い）
- **APIがポスターを持っていない作品**（題名を書いただけのSVGが返る。使わない）
- URLが失効している／取得に失敗した（下の11節）

> **文字だけの版も必ず保守すること。** 提供元との契約を終えたら、
> サイトはこの版だけで成立する必要がある。
> コードは `make-sections.mjs` の `buildSvg()` に両方入っている。
...
| `site/public/sections/` | ポスターを焼き込んだ完成画像 | **入れない**（毎ビルド再生成） |
