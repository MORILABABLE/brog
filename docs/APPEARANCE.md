...
```css
.layout { display: block; }          /* 既定。1200px 未満は従来と同一 */
...
上2ブロックまで広告と称することになり、逆にPR表記を外すと
景品表示法（ステマ規制）に触れる。
...
> **追従するのは PR枠だけ。** 上2ブロックはページと一緒に流れる。
>
> 枠全体を `sticky` にすると、枠が画面より高いときに上端で貼り付き
> **下端のPR枠に永久に手が届かなくなる**。枠の中をスクロールさせれば届くが、
> **読者がスクロールを2回する**ことになる。
> 上2ブロックを流し、PR枠（約230px＝どんな画面にも収まる）だけを追従させるのが、
...
`src/assets/services/<キー>.png` を置くだけで、そのカードに56×56pxの絵が出る。
...
> 併せて `.rail-card { min-height: 72px }` を入れてあるので、
> 万一折り返しても高さは下回らない。
...
| 変えたいこと | 直す場所 |
|---|---|
| 枠を出す画面幅の下限 | `.layout` のメディアクエリ `75rem`（下げすぎ注意・下記） |
...
| ブロックの見出し文言 | `FollowRail.astro` の `.block-title` を持つ `<p>` |
| **枠を消す** | `BaseLayout.astro` から `<FollowRail />` を外す |

> **狭い画面（1200px未満）では枠ごと消える。**
