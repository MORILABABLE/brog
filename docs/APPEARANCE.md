...
> **枠が画面より高くなると、sticky の下端（＝PR枠）に手が届かなくなる。**
> `.inner` に `max-height: calc(100vh - 2rem)` と `overflow-y: auto` を入れて、
> 収まらないときだけ枠の中がスクロールするようにしてある。外さないこと。
...
| 新着記事の本数 | `FollowRail.astro` の `RECENT_POSTS`（既定4） |
