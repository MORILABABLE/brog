...
> ★ **「直したのに反映されない」ときは `npm run dev:fresh` を使う。**
>
> **記事の描画結果（HTML）がキャッシュされる。** `.md` が変わらないかぎり
> 再描画されないので、次のどちらも**手元では反映されない**。
>
> - `site/plugins/rehype-*.ts` を直した
> - プラグインが読む `data/` を更新した（`events` / `availability` / `titles` …）
>
> `dev:fresh` / `build:fresh` がキャッシュを消してから起動する
> （`site/scripts/clear-cache.mjs`）。
>
> ★ **2026-09-06 に実際に踏んだ。** 新しく起動した dev だけが古い表を出し、
> **前日から動いていた dev のほうが正しい表を出していた**
> （動いている dev は HMR で作り直されるが、起動時はキャッシュを読むため）。
>
> ★ **Cloudflare のビルドは毎回まっさら。** 公開されるものは常に最新で、
> この問題は手元でしか起きない。
>
> ★ 古い dev サーバが残っていると、**別のポートで起動して古い画面を見続ける**
> ことがある（Astro は 4321 が埋まっていると 4322 へずれる）。
> 見ているポート番号を確認すること。
...
| **文章の書き方の決まり**（全記事タイプ共通） | `theme-packs/streaming-jp/templates/writing.md` |
...
### writing.md — 文章の書き方（全記事タイプ共通）

**構成は記事タイプごとのファイルにあり、こちらは「どう書くか」だけ。**
1枚にまとめてあるのは `naming.md` と同じ理由で、
6つのテンプレートに書き写すと必ずどれかが古くなるため。

添削で出た指摘のうち、**記事タイプをまたいで効くもの**はここに足す。

| 節 | 決まり |
|---|---|
| 1 | 記事の作りを読者に説明しない（並び順の宣言・素材の偏りもここ） |
| 2 | 配信の裏側（契約・権利）を推測しない |
| 3 | 作品の魅力は「事実」で書く（リサーチの使い方） |
| 4 | 事実と、事実でないものを混ぜない |
| 5 | 節の中で最初に触れる2作は、表の題名を省略せずに書く（画像が付く条件） |
| 6 | 出演者の出しかた |
| **7** | **数字と作品名だけで段落を作らない**（2026-09-10 追加） |
| **8** | **あらすじは1作ずつ、具体で書く**（2026-09-10 追加） |
| **9** | **同じ名前・同じ褒め言葉を繰り返さない**（2026-09-10 追加） |

★ 1節・7節は品質ゲート（`article-types/shared.ts` の `styleIssues()`）が
機械的にも見ている。**言い回しを足すときは両方に足す。**
...
## 月が過ぎた月次記事は、機械が名乗り直す（2026-09-03〜）

**月次記事は書き直さない。** 直すのは frontmatter の2行だけで、しかも**自動で行われる**。
手で何かする必要は普段ない。ここは「なぜ本文を直さないのか」を確かめたいとき用。

`/refresh` が受け持つのは**月を名乗らない記事**（シリーズの保存版）だけ。
月次記事はそもそも古くならない。名乗っている範囲が違う。

| | 月を名乗っているか | 9月に読んだときどうなるか |
|---|---|---|
| タイトル `【2026年8月】…配信終了する作品32本` | ○ | 8月の記録として正しい |
| 見出し `## 8月11日：…が一斉に終了` | ○（日付） | 正しい |
| 本文 `8月9日時点では32本が…` | ○（基準日） | 正しい |
| 本文の「観ておきましょう」 | ○（前後が日付） | 8月時点の呼びかけとして読める |
| **バッジ `配信終了予定`**（`category: leaving`） | **×** | **嘘。9月の本物の終了予定記事と同じ色で並ぶ** |

バッジは記事一覧・追従枠・左枠・記事ページ・サイト内検索の5か所に出る
（`site/src/config.ts` の `CATEGORIES`）。**記事の外に置かれる唯一の「いま」の主張**で、
古くなるのはここだけ。だから直すのもここだけでよい。

```
npm run retire            何が変わるかを出すだけ
...
**作品を1本も見ていないから。** 見ているのは frontmatter のタグだけで、本文も収集データも読まない。
根拠は `leaving` の `select()` が「対象月に終了」かつ「終了日が未来」で絞っていること。
**その月が終われば、載っている全作品の終了日が過ぎたと確定する。** 数え直す余地が無い。

- 何度実行しても結果は同じ（すでに直っている記事は差分が出ない）
- 対象は「いまより前の月」全部なので、実行が1回飛んでも翌日が拾う
- `heroImage` も出典も消えない（`--apply` と違い、frontmatter を組み立て直さない）
- `updatedDate` は足さない。本文が1文字も変わっていないのに
  サイトマップの `lastmod` を動かすと、鮮度の信号が当てにされなくなる

### 規則を足すとき

記事タイプに `retire` を宣言するだけ（`theme-packs/streaming-jp/article-types/leaving.ts`）。
CLI もワークフローも触らない。規則は**タグで引く**ので、同じタグを付ける特報も一緒に移る。

★ **配信開始側には宣言しないこと。** 「終了予定日が過ぎた＝終了した」は言えるが、
  「配信開始予定日が過ぎた＝始まった」は言えない。告知どおりに始まったかを当サイトは
  観測していない。`配信開始予定` のタグが残るのは**確かめていないことを書かない**ためで、
  直し忘れではない（`pipeline/core/article.ts` の `retire`）。
