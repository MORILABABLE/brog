...
| 右の追従枠 | ✅ 常設ページへのナビ＋広告カード。1280px以上でのみ表示。サムネイルは `site/src/assets/services/` に置くだけ（[APPEARANCE.md](./APPEARANCE.md)） |
...
- 追従枠のサムネイル画像が**未配置**（文字だけで表示中）。
  `site/src/assets/services/{netflix,prime-video,disney-plus,stats}.png` を置けば出る
