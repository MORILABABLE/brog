...
| 画像 | ✅ 記事カード（OG）と本文セクション画像を**ビルド時に自動生成**。セクション画像には**作品ポスターを合成**（提供元の許諾済み・[11節](./APPEARANCE.md#11-作品ポスターの取り扱い許諾取り直し契約終了)）。記事冒頭の `heroImage` は未使用 |
...
> **記事冒頭の横長画像（frontmatter の `heroImage`）はまだ空のまま。**
> 縦長のポスターを 16:7 に切っても絵にならないため、横位置のキーアート
> （`horizontalBackdrop`）が要る。収集済みのイベントには入っていないが、
> 2026-08-25 に収集側へ保存を足したので、**次回以降の `npm run collect` で
> 集まる作品からは使えるようになる**（追加のAPIコストは無い）。
