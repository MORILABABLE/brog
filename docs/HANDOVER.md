...
| 右の追従枠 | ✅ 常設ページのカード（1ページ1枚）＋広告カード。1200px以上でのみ表示。サムネイルは `site/src/assets/services/` に置くだけ（[APPEARANCE.md](./APPEARANCE.md)） |
...
- 追従枠のサムネイル画像が**未配置**（サービス名の頭文字で表示中）。
  `site/src/assets/services/{netflix,prime-video,disney-plus}.png` を置けば出る
- `/stats`（見放題の増減）は**一覧・追従枠から外してある**。読者に用途が伝わりにくいため。
