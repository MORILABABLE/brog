...
3. `mitokou` で検索し、`.com` が空いていることを確認
   （2026-08-01 時点で RDAP により未登録を確認済み。年額 約$10.5）
...
**すでに `mitokou.com` に設定済み**。別のドメインにした場合のみ書き換える。
...
2. 取得したドメイン（例: `mitokou.com`）を入力
3. Cloudflare Registrar で取得していれば、**DNSレコードは自動で作成される**
4. 数分〜十数分で HTTPS 込みで公開される（証明書も自動）

`www` ありでもアクセスさせたい場合は、`www.mitokou.com` も同様に追加する。
