 ...
 * 絵を入れない節。
 *
 * ★ 「全終了作品リスト」「他のサービスで探す」は **2026-09-19 に廃止した節**だが、
 *   **外さないこと。** 公開済みの記事にはまだ残っていて、書き直すまで本文にある。
 *   新テンプレで残るのは「まとめ」だけ（小段落1・2はどちらも絵が入る）。
 */
...
function pickHighlights(section) {
   ...
   *
   * ★ **「評価」の列を持たない記事では、表の並び順がそのまま順位になる**
   *   （2026-09-08）。シリーズ記事の表は「評価」を「出演者」に置き換えたので、
   *   ここで読める得点が無くなった。`rating` が空だと `parseInt` は NaN → 0 に
   *   落ち、`sort` が安定ソートなので**表に書いた順**で埋まる。
   *   記事側は評価の高い順に行を並べる決まりなので、結果は今までと同じになる。
   *   **評価を素材から消したわけではない**（記事に出さないだけ）。
   */
