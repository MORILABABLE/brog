 ...
 *
 * ★ このファイルは3か所から読まれる。**1か所だけ直さないこと。**
 *     scripts/make-thumbs.mjs   … 表の作品サムネイルを生成する（key ごとに1枚）
 *     src/lib/work-links.ts     … 作品からどの画像を使うかを引く
 *     scripts/make-sections.mjs … 記事のヘッダー画像。2通りの使い方をしている
 *                                 節タイルの下地は `genreMotif`（薄く敷いて上に題名）、
 *                                 ポスターが1枚も無い記事の絵は `genreSvg` を
 *                                 ポスター大（480×720）でそのまま使う
 */
 ...
 *
 * genreSvg() は絵柄を主役に据えた1枚を返す。
 * 節タイル（make-sections.mjs の buildTileSvg）は**同じ絵柄を薄く敷いて
 * 上に作品名を載せる**ので、出来上がりではなく部品が要る。ここはその窓口。
 *
 * ★ 絵柄だけを見せたいとき（記事のヘッダー用の汎用画像）は genreSvg() でよい。
 *   viewBox が 100×150＝2:3 なので、ポスターと同じ比率でそのまま拡大できる。
 *
