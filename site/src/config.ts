...
/**
 * 常設ページ（`/leaving/<サービス>` `/arrivals/<サービス>`）であることを示すしるし。
 * **カテゴリバッジの右隣に並ぶ**（2026-09-05 追加）。
 *
 * ★ 出る場所は3つ。**文言を変えるなら3か所とも同じように変わる**（この定数を見ている）。
 *   1. 一覧の中の常設カード（EvergreenCard.astro）
 *   2. 常設ページ自身の見出しの下（pages/leaving・arrivals・calendar の各 [service].astro）
 *   3. **左の枠の常設カード**（LeftRail.astro。2026-09-15 追加）
 *   3 を足したとき、同じカードに出していた更新日（`【◯月◯日更新】`）は外した。
 *   「このページは古くなるのか」に答えるものが2つ並ぶ必要はない。
 *
 * ■ なぜ要るか
 * 常設ページのカード（EvergreenCard）は、記事カード（PostCard）と
 * **わざと同じ体裁にしてある**（2026-08-23。別の箱にすると一覧の中で浮くため）。
 * ところが揃えた結果、一覧に並んだとき**記事と見分けが付かなくなった。**
 * 読者にとって両者はまったく別物で、
 *
 *   記事     … 書いた日のスナップショット。日付が古くなる。解説がある
 *   常設ページ … 収集のたびに中身が入れ替わる。**古くならない。URLが変わらない**
 *
 * 期待するものが違うので、押す前に分かる必要がある。
 *
 * ■ なぜ色を足さないか
 * 色は**カテゴリ（何が起きたか）の意味に使い切っている**（styles/global.css の :root）。
 * 5色目を足すと、隣り合う2つのバッジが別々の色の意味を主張してどちらも読めなくなる。
 * ジャンルのバッジと同じく**枠線だけの形**にして、意味は文字に持たせる。
 * ただしジャンル（副次的な情報）と違い、こちらは「このページは記事ではない」という
 * **一次情報**なので、枠線と文字をアクセント色にして一段強く出す。
 *
 * ★ 文言に更新の頻度を書かないこと（「毎日更新」など）。
 *   実際に動くのは収集が走ったときで、サービスによって頻度が違う。
 *   いつ時点かは `evergreenTitle()` が見出しの頭に出している。
 */
export const EVERGREEN_BADGE = {
  label: '随時更新',
  /*
   * ★ 見出し下に添えていた説明の1行（`note`）は**撤去した**（2026-09-09）。
   *   「配信データをサービスごとに随時更新し一覧にしています。…」という文で、
   *   常設ページ3種（終了予定・新着・カレンダー）の見出しのすぐ下に出ていた。
   *   バッジの「随時更新」と見出しの「◯月◯日時点」で同じことは伝わっており、
   *   本題（配信カレンダーの升目）を1画面ぶん押し下げていた。
   *   **書き戻すなら3枚すべてに戻すこと。**1枚だけ違う形にしない。
   */
} as const
...
/**
 * 月別まとめ（`/archive`）。**4本目の軸**（2026-09-05 に追加）。
 *
 * ■ なぜ要るか
 * このサイトの記事は**ほぼすべてが月次**（「2026年9月に終了予定の作品」）なのに、
 * 月で辿る道が1本も無かった。カテゴリ×サービスの一覧は月をまたいで
 * 新しい順に積むだけなので、**「9月のNetflixの記事はどれか」**に答えるには
 * 読者が日付を目で拾うしかない。月が増えるほど下へ流れて見つからなくなる。
 *
 * ■ 3つの軸との違い
 *   カテゴリ … 何が起きたか（終了まわり／新着）
 *   サービス … どのサービスの話か
 *   ジャンル … 何を観る話か
 *   **月**   … **いつの話か。** 記事の寿命がそのまま1か月なので、
 *              このサイトでは「号」に近い意味を持つ
 *
 * ■ 月をどこから取るか
 * 記事の `tags` に入っている `2026年9月` の形のタグ**だけ**を見る
 * （`pubDate` は使わない。9月の記事は8月末に出るため）。
 * 判定は src/lib/archive.ts の `monthOf()` 1か所。
 *
 * ★ **シリーズ記事は月を名乗らない**ので、月別まとめに出ない。
 *   保存版であって特定の月のものではないため（`article-types/series.ts`）。
 *   入口は右の枠と `/series`。
 *
 * ★ ヘッダーのメニューは 2026-09-05 に「監督・出演者」から入れ替えた。
 *   人物ページへの入口は左の枠とフッターが持つ（Header.astro の説明）。
 */
export const ARCHIVE = {
  /** URLの根。`/archive/2026-09` `/archive/2026-09/netflix` */
  base: '/archive',
  /** ヘッダーのメニューの文言。**1行に収まる長さを守ること**（Header.astro の幅の内訳） */
  label: '月別まとめ',
  heading: '月別まとめ',
  description: '月ごとに、どのサービスで何が配信開始・配信終了したかの記事をまとめています。',
  /**
   * ヘッダーのメニューに出す月の数。**新しい順にこの数だけ。**
   * 全部への経路は「すべて見る」が持つ（サービスのメニューと同じ作り）。
   */
  menuMonths: 6,
} as const
 ...
 *   ジャンル … 何を観る話か（アニメ／洋画／邦画）。**全記事が1つ以上持つ**
 *
 * ★ **ジャンルは1つに絞らない**（2026-09-15 に単数の `genre` から変えた）。
 *   「Netflixで配信開始の作品199本」のようなサービス軸の記事は
 *   アニメも洋画も邦画も全部入っているので、1つだけ名乗らせると嘘になる。
 *   かといって名乗らせないでいたら、**32本中29本がバッジを持たない**状態になり、
 *   読者は一覧で「何を観る話か」をタイトルから読み取るしかなかった。
 *   **入っているものを全部並べる**形にすれば、どちらの問題も起きない。
 *   frontmatter は `genres`（配列・本数の多い順）と `genreDetail`（任意）。
 *   数え方は theme-packs/streaming-jp/genres.ts の `summarizeGenres()`、
 *   バッジの文言は src/lib/genres.ts。（軸の考え方は pipeline/core/article.ts の `Axis`）
 *
 * ★ **サイトに出すのはこの3つの表記だけ。** 詳細（SF・アクションなど）は
 *   括弧に入れて基本ジャンルに添える形にとどめ、4つ目のラベルを足さない。
 *   バッジが並ぶ行にはカテゴリの色つきバッジも載るので、
 *   語彙を増やすほど「どれが分類でどれが補足か」が読めなくなる。
 *
 ...
 *   バッジと枠の中は短いほうがよく（アニメ／洋画／邦画。**最大3枚並ぶ**）、
...
export const AFFILIATE = {
   ...
   *
   * ⛔ **2026-09-19、afb の U-NEXT 提携は不合格。この変数は当面ずっと空**
   *   （＝ U-NEXT の広告は1枚も出ない）。docs/AFFILIATE.md 14節。
   *   **消さないこと。** **別ASP（A8.net 等）で通った日**にここへ href を入れれば全ページに戻る
   *   （afb では再審査されない。変数名は afb 由来だが、中身はただのURL）。
   *
