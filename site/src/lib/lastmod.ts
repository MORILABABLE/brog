...
let cache: { data?: string; posts: Map<string, string> } | null = null

function load(): { data?: string; posts: Map<string, string> } {
  if (!cache) cache = { data: dataUpdatedAt(), posts: postDates() }
  return cache
}
