 ...
 *
 * ⛔ **2026-09-19、afb の U-NEXT 提携は不合格。このファイルはいま休眠中。**
 *    `PUBLIC_AFB_UNEXT_LP` が空なので `unextAd()` は常に null を返し、
 *    **U-NEXT の広告は1バイトも出ていない**（＝ afb の枠はすべて Hulu が埋めている）。
 *    ★ **afb は落ちた媒体を再審査しない。** 復活するとしたら別ASP（A8.net 等）経由で、
 *    **その日に差し替えるのは LP の href と afb.ts の `withSlot()` だけ**（docs/AFFILIATE.md 14-3）。
 *    **消さないこと。** 中身は ASP ではなく **U-NEXT のガイドライン対応**なので、
 *    どのASPで通っても丸ごと効く。経緯と要因の推測は docs/AFFILIATE.md 14節。
 *    ★ `unext-ng.ts`（掲載NGの判定）は **Hulu が使っているので現役**。こちらは止めない。
 *
