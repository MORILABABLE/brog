...
#### 見出しは必ず基準日から始める（2026-08-25〜）

常設ページは**公開日を持たない。** 中身だけが入れ替わり続けるので、
読者から見ると「いつの情報か分からないページ」になりやすい。
そこで基準日（＝そのページに載っている作品を最後に確認した収集時刻）を
必ず先頭に出す。**今後ページを増やすときも同じ形にすること。**

| 出る場所 | 形 | 例 |
|---|---|---|
| ページの `<title>` / `<h1>`、一覧カードの見出し | `【〈基準日〉時点】〈素のタイトル〉` | 【2026年8月25日時点】Netflixで配信終了予定の作品一覧 |
| 常設枠（左の枠）のカード | `【〈基準日〉更新】〈サービス名〉` | 【2026年8月25日更新】Netflix |

組み立ては `site/src/lib/evergreen.ts` の `evergreenTitle()` と `evergreenStamp()`
**だけ**が行う。ページ・カード・左の枠が別々に文字列を作ると、
同じページが場所によって違う日付を名乗ることになる。

> **素のタイトルに「最近」のような相対表現を入れない。**
> 日付と並ぶと、収集が止まったときに「最近」と古い日付が矛盾して見える。
> `/arrivals/…` のタイトルから「最近」を外したのはこのため。
...
| 手段 | 単価 | 位置づけ |
|---|---|---|
| **Hulu の入会案件**（afb） | 917円/件 | 🟢 **主力**（2026-09-14 提携）。**作品名を入れた広告は作れない**ので、LP導線だけ |
| **Amazonアソシエイト** | 紹介料＋Prime無料体験 500円/件 | 🟢 **主力**。対象4社で唯一提携できる。24時間クッキーが実体 |
| AdSense | 数十〜数百円/日 | 🔶 **審査中**（2026-09-19 申請）。3本目の柱になる予定 |
| U-NEXT の入会案件 | 1,320〜1,430円/件 | ❌ **afb は不合格・再申請不可**（2026-09-19）。実装は残してある。**やるなら別ASP**（AFFILIATE 14-3） |
| DMM TV の入会案件 | 1,539〜1,691円/件 | **当面見送り**。FANZA同一基盤でAdSense審査にリスク（3.7） |
| Apple TV+ / Disney+ の入会案件 | 1,299〜1,720円/件 | Apple TV+ は招待制。**Disney+ は afb で審査中**だが、**パック案件が落ちており見込みは低い**（AFFILIATE 14-2 の F） |

> **2026-09-19 の訂正。** この表は長らく「U-NEXT / Hulu が主力」と書いていたが、
> **U-NEXT は afb の提携審査に落ち、afb では再申請できない。**
> 主線は **Hulu・Amazon・AdSense（審査中）** の3本。
> 落ちた要因の推測と次のASPの進め方は [docs/AFFILIATE.md 14節](./docs/AFFILIATE.md)。
