...
| Search Console のインデックス登録（未登録の内訳と対処） | [docs/INDEXING.md](./docs/INDEXING.md) |
...
`images` は**公開済みの記事に手を入れる唯一のワークフロー**（`retire` の
frontmatter 2行を除く）。配信開始前に書いた記事は作品ポスターが1枚も無いまま
出るが、配信が始まればAPIが画像を返すので、**あとから本物のポスターに差し替える**。
書き換えるのは本文の `![…]()` の行と frontmatter の `heroImage:` だけで、
差し替えるものが無い日は何もコミットしない。
...
| `npm run check:ads` | **ビルド済みのサイトを afb 広告主（U-NEXT / Hulu）のガイドラインで検査する**（禁止表現・掲載NG作品・必須文言・枠の重複）。`cd site && npm run build` のあとに実行する。`check:unext` は同じものの別名 |
...
**あとから画像が取れるようになったときの差し替えは自動。**
配信開始前に書いた記事（`upcoming`）は作品ポスターが1枚も無いまま出るが、
配信が始まればAPIが画像を返すので、`images` ワークフローが毎日
本物のポスターに差し替えてコミットする（→ [docs/APPEARANCE.md 10節](./docs/APPEARANCE.md)）。
